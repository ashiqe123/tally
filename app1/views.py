...
def sales_voucher(request):
    if 't_id' in request.session:
        ...
        if request.method=="POST":
          ...
          inv_n=request.POST.get('invno')
          party_ac=request.POST.get('myselect') #party ac dropdown id
          print("party ac name",party_ac)
          date=request.POST.get('date')
          sale_led=request.POST.get('myselect2') #sale ledger dropdown id
           
          if sl_in_id=="":
            sal_one_tab=sales_invoice(inv_num=inv_n,party_ac_name=party_ac,sales_ledger=sale_led,date=date,company=co,dispatch_id=dis_id ,party_detail_id =par_id )
          else:
            sal_one_tab=sales_invoice(id=sl_in_id,inv_num=inv_n,party_ac_name=party_ac,sales_ledger=sale_led,date=date,company=co,dispatch_id=dis_id ,party_detail_id =par_id )
          sal_one_tab.save()
          print("Sales one save")
